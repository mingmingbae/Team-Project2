...
<script src="http://dmaps.daum.net/map_js_init/postcode.v2.js"></script>
 <script>
   ...
   //웹브라우저가 jsp파일의 모든 HTML태그들을 읽어들였을때~~ function함수가 실행되게 선언 !
   window.onload=function()
    {
      selectBoxInit(); //아래의 함수 호출!
    }
    ...
    //유선전화의 국번을 저장
     ...
     //유선 전화 국번의 <option>태그들 중에 하나가 선택되도록  설정
     for(var i=0; i<optionTel1.length;i++){
      ...
      //휴대폰 국번의 <option>태그들 중에 하나가 선택되도록  설정
...
function fn_remove_member() {
	...
	if(window.confirm("탈퇴하시겠습니까?")){
		$.ajax({
			...
			url : "${contextPath}/member/removeMember.do",
			data : {
				mem_id:member_id,
			},
			success : function(data) {
				console.log(data)
				if(data.trim()=='탈퇴성공'){
					alert("회원 탈퇴 되었습니다. 그 동안 이용해주셔서 감사합니다.");
					location.href="${contextPath}/main/main.do";
				}else if(data.trim()=='탈퇴실패'){
					alert("다시 시도해 주세요.");	
				}
			},
			error : function(data, textStatus) {
...
<form name="frm_mod_member">	
	<div id="detail_table">
		<table>
			<tbody>
				<tr class="dot_line">
					...
					<td>
						<input name="member_id" id="member_id" type="text" size="20" value="${sessionScope.memberInfo.member_id }"  disabled/>
					</td>
					 <td>
					</td>
				</tr>
		...
		<tr>
			...
			<td >
				<input name="btn_remove_member" type="button"  value="회원 탈퇴" onclick="fn_remove_member()">
			</td>			
		</tr>
	</table>
	</div>
